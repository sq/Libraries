...
namespace Squared.Threading.CoreCLR
{
    ...
    /// These segments are linked together to form the unbounded <see cref="LowAllocConcurrentQueue{T}"/>.
    /// </summary>
    [DebuggerDisplay("Capacity = {Capacity}")]
    internal sealed class LowAllocConcurrentQueueSegment<T>
    {
...
#pragma warning disable 0649 // some builds don't assign to this field
        ...
        internal LowAllocConcurrentQueueSegment<T> _nextSegment; // SOS's ThreadPool command depends on this name
#pragma warning restore 0649
        ...
        /// </param>
        internal LowAllocConcurrentQueueSegment(int boundedLength)
        {
            // Validate the length
                ...
                if (diff == 0)
                {
                    ...
                    if (Interlocked.CompareExchange(ref _headAndTail.Head, currentHead + 1, currentHead) == currentHead)
                    {
                        ...
                        if (!Volatile.Read(ref _preservedForObservation))
                        {
                            ...
                            // FIXME: Need a version of this for net4x
        ...
        public bool TryEnqueue(ref T item)
        {
            ...
            while (true)
            {
                ...
                if (diff == 0)
                {
                    ...
                    // FIXME: net4x does support Thread.Abort, so this is broken.
