...
namespace Squared.Threading.CoreCLR
{
    /// <summary>
    ...
    /// <remarks>
    /// All public and protected members of <see cref="LowAllocConcurrentQueue{T}"/> are thread-safe and may be used
    /// concurrently from multiple threads.
    /// </remarks>
    [DebuggerDisplay("Count = {Count}")]
    public class LowAllocConcurrentQueue<T> : IReadOnlyCollection<T>, ICollection
    {
        ...
        private volatile LowAllocConcurrentQueueSegment<T> _tail;
        /// <summary>The current head segment.</summary>
        private volatile LowAllocConcurrentQueueSegment<T> _head; // SOS's ThreadPool command depends on this name

        /// <summary>
        /// Initializes a new instance of the <see cref="ConcurrentQueue{T}"/> class.
        /// </summary>
        public LowAllocConcurrentQueue()
        {
            _crossSegmentLock = new object();
            _tail = _head = AllocateSegment(InitialSegmentLength);
        }
        ...
        public LowAllocConcurrentQueue(IEnumerable<T> collection)
        {
            if (collection == null)
            {
            ...
            if (collection is ICollection<T> c)
            {
                ...
                if (count > length)
                {
                    length = Math.Min(Util.Arithmetic.NextPowerOfTwo(count), MaxSegmentLength);
                }
            }

            // Initialize the segment and add all of the data to it.
            _tail = _head = AllocateSegment(length);
        ...
        private LowAllocConcurrentQueueSegment<T> AllocateSegment (int length) =>
            new LowAllocConcurrentQueueSegment<T>(length);
        ...
        public T[] ToArray()
        {
            ...
            SnapForObservation(out var head, out int headHead, out var tail, out int tailTail);
        ...
        private static int GetCount(LowAllocConcurrentQueueSegment<T> s, int head, int tail)
        {
        ...
        private static long GetCount(LowAllocConcurrentQueueSegment<T> head, int headHead, LowAllocConcurrentQueueSegment<T> tail, int tailTail)
        {
        ...
        /// </summary>
        private void SnapForObservation(out LowAllocConcurrentQueueSegment<T> head, out int headHead, out LowAllocConcurrentQueueSegment<T> tail, out int tailTail)
        {
        ...
        private static T GetItemWhenAvailable(LowAllocConcurrentQueueSegment<T> segment, int i)
        {
        ...
        private static IEnumerator<T> Enumerate(LowAllocConcurrentQueueSegment<T> head, int headHead, LowAllocConcurrentQueueSegment<T> tail, int tailTail)
        {
            Debug.Assert(head._preservedForObservation);
            Debug.Assert(head._frozenForEnqueues);
            Debug.Assert(tail._preservedForObservation);
        ...
        private void EnqueueSlow(ref T item)
        {
            while (true)
            {
                var tail = _tail;
                ...
                lock (_crossSegmentLock)
                {
                    if (tail == _tail)
                    {
                        ...
                        var newTail = AllocateSegment(nextSize);
            ...
            while (true)
            {
                ...
                lock (_crossSegmentLock)
                {
                    if (head == _head)
                    {
                        ...
                        // No pool is necessary at present based on basic testing; this version of ConcurrentQueue
                        //  is MUCH better than the one in net4.x
                        // ReturnToPool(head);
                    }
                }
            }
        ...
        private bool TryPeek(out T result, bool resultUsed)
        {
            ...
            while (true)
            {
                ...
                var next = Volatile.Read(ref s._nextSegment);
