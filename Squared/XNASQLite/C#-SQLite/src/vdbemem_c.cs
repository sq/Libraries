...
return rc;
    ...
    */
    static int sqlite3VdbeMemGrow( ref Mem pMem, int n, int preserve )
    {
...
*/
    ...
    */
    static int sqlite3VdbeMemFinalize( ref Mem pMem, FuncDef pFunc )
    {
      ...
      if ( ALWAYS( pFunc != null && pFunc.xFinalize != null ) )
      {
        ...
        Debug.Assert( ( pMem.flags & MEM_Null ) != 0 || pFunc == pMem.upDef );
        Debug.Assert( pMem.db == null || sqlite3_mutex_held( pMem.db.mutex ) );
        //memset(&ctx, 0, sizeof(ctx));
        ctx.s.flags = MEM_Null;
        ctx.s.db = pMem.db;
        ctx.pMem = new MemRef(ref pMem);
        ctx.pFunc = pFunc;
        pFunc.xFinalize( ctx );
    ...
    */
    static void sqlite3VdbeMemRelease( ref Mem p )
    {
      sqlite3VdbeMemReleaseExternal( ref p );
...
return SQLITE_NOMEM;
    ...
    */
    ...
    int amt,          /* Number of bytes to return. */
    bool key,         /* If true, retrieve from the btree key, not data. */
    ref Mem pMem          /* OUT: Return data in this Mem structure. */
    )
    {
