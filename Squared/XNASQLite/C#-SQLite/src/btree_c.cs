...
*/
    static readonly byte[] cellSizePtrBuf = new byte[13];
    static byte[] fetchPayloadScratchBuffer = new byte[4096];
    ...
    */
    ...
    {
      int rc;                    /* Status code */
      UnpackedRecord pIdxKey = new UnpackedRecord();   /* Unpacked index key */

      if (pKey != null)
      {
        Debug.Assert(nKey == (i64)(int)nKey);
        sqlite3VdbeRecordUnpack(pCur.pKeyInfo, (int)nKey, pKey, ref pIdxKey, 16);//sizeof( aSpace ) );
        if (pIdxKey.IsEmpty) return SQLITE_NOMEM;
      }

      rc = sqlite3BtreeMovetoUnpacked(pCur, ref pIdxKey, nKey, bias != 0 ? 1 : 0, ref pRes);
    ...
    */
    ...
    static u16 cellSizePtr(MemPage pPage, int iCell)
    {
      ...
      if (iCell < 0)// Overflow Cell
        btreeParseCellPtr(pPage, pPage.aOvfl[-(iCell + 1)].pCell, 0, ref info);
      else
        btreeParseCellPtr(pPage, pPage.aData, iCell, ref info);
      return info.nSize;
    }
    ...
    {
      CellInfo info = new CellInfo();
      btreeParseCellPtr(pPage, pCell, offset, ref info);
      return info.nSize;
    }
...
const Pgno iLastPg = Pgno.MaxValue;
    ...
    */
    ...
    )
    {
      ...
      using (var zDbHeader = BufferPool<byte>.Allocate(100)) { /* Database header content */
...
p.sharable = 1;
      ...
      if (pBt == null)
      {
        ...
        */
        ...
        if (rc == SQLITE_OK)
        {
          rc = sqlite3PagerReadFileheader(pBt.pPager, zDbHeader.Length, zDbHeader.Array);
        }
        if (rc != SQLITE_OK)
        {
          goto btree_open_out;
        ...
        pBt.pageSize = (u16)get2byte(zDbHeader.Array, 16);
...
#if !SQLITE_OMIT_AUTOVACUUM
          pBt.autoVacuum = sqlite3Get4byte(zDbHeader.Array, 36 + 4 * 4) != 0;
          pBt.incrVacuum = sqlite3Get4byte(zDbHeader.Array, 36 + 7 * 4) != 0;
#endif
        }
...
sqlite3BtreeTripAllCursors(p, rc);
    ...
    */
    ...
    static void assertCellInfo(BtCursor pCur)
    {
      ...
      Debug.Assert(info.Equals(ref pCur.info));//memcmp(info, ref pCur.info, sizeof(info))==null );
    }
#else
...
return SQLITE_ABORT;
    ...
    */
    ...
    )
    {
      ...
      int scratchSize = pCur.info.nSize - pCur.info.nHeader;
      
      aPayload = fetchPayloadScratchBuffer;
      if (aPayload.Length < scratchSize)
        aPayload = fetchPayloadScratchBuffer = new byte[scratchSize];
...
** to the last entry in the b-tree. */
    ...
    */
    ...
    ref UnpackedRecord pIdxKey,  /* Unpacked index key */
    ...
    )
    {
      ...
      Debug.Assert((pIdxKey.IsEmpty) == (pCur.pKeyInfo == null));
      ...
      ** to move to, then just return without doing any work */
      ...
      Debug.Assert(pCur.apPage[0].intKey != 0 || !pIdxKey.IsEmpty);
      for (; ; )
      {
        int lwr, upr;
        Pgno chldPg;
        ...
        ** a moveToChild() or moveToRoot() call would have detected corruption.  */
        ...
        Debug.Assert(pPage.intKey == ((pIdxKey.IsEmpty) ? 1 : 0));
        lwr = 0;
        upr = pPage.nCell - 1;
        if (biasRight != 0)
        {
          ...
          else
          {
            ...
            */
            ...
            if (0 == (nCell & 0x80) && nCell <= pPage.maxLocal)
            {
              ...
              ** b-tree page.  */
              c = sqlite3VdbeRecordCompare(nCell, pPage.aData, pCell + 1, ref pIdxKey); //c = sqlite3VdbeRecordCompare( nCell, (void*)&pCell[1], pIdxKey );
            }
            ...
            )
            {
              ...
              ** fits entirely on the main b-tree page.  */
              c = sqlite3VdbeRecordCompare(nCell, pPage.aData, pCell + 2, ref pIdxKey); //c = sqlite3VdbeRecordCompare( nCell, (void*)&pCell[2], pIdxKey );
            }
            else
            {
              ...
              ** buffer before VdbeRecordCompare() can be called. */
              ...
              c = sqlite3VdbeRecordCompare(nCell, pCellKey, ref pIdxKey);
...
*/
        ...
        ** is still writeable */
        ...
        {
          csSQLite.memset(pPayload, pPayloadIndex, 0, n);
        }
        nPayload -= n;
...
if (false)
    ...
    */
    ...
    )
    {
      ...
      Buffer<MemPage> apCopy = BufferPool<MemPage>.Allocate(NB);   /* Private copies of apOld[] pages */
      Buffer<MemPage> apNew = BufferPool<MemPage>.Allocate(NB + 2);/* pPage and up to NB siblings after balancing */
      int pRight;                  /* Location in parent of right-sibling pointer */
      Buffer<int> apDiv = BufferPool<int>.Allocate(NB - 1);        /* Divider cells in pParent */
      Buffer<int> cntNew = BufferPool<int>.Allocate(NB + 2);       /* Index in aCell[] of cell after i-th page */
      Buffer<int> szNew = BufferPool<int>.Allocate(NB + 2);        /* Combined size of cells place on i-th page */
      Pgno pgno;                   /* Temp var to store a page number in */
      Buffer<byte[]> apCell = new Buffer<byte[]>();
      Buffer<u16> szCell = new Buffer<u16>();
...
*/
      ...
      */
      ...
      while (true)
      {
        rc = getAndInitPage(pBt, pgno, ref apOld.Array[i]);
        if (rc != 0)
        {
          Array.Clear(apOld.Array, 0, i + 1);
...
apDiv[i] = &aOvflSpace[apDiv[i]-pParent.aData];
      ...
      ** alignment */
      ...
      apCell = BufferPool<byte[]>.Allocate(nMaxCells); //apCell = sqlite3ScratchMalloc( szScratch );
      szCell = BufferPool<u16>.Allocate(nMaxCells); //(u16*)&apCell[nMaxCells];
      ...
      */
      ...
      for (i = 0; i < nOld; i++)
      {
        ...
        ** process of being overwritten.  */
        ...
        {
          u16 sz = (u16)szNew[i];
          using (var pTemp = BufferPool<byte>.Allocate(sz + leafCorrection)) {
          Debug.Assert(nCell < nMaxCells);
          szCell[nCell] = sz;
...
if (false)
      ...
      */
      ...
      for (i = 0; i < nNew; i++)
      {
        ...
        assemblePage(pNew, cntNew[i] - j, apCell.Array, szCell.Array, j);
