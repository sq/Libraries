...
namespace CS_SQLite3
{
  ...
  public partial class csSQLite
  {
    ...
    */
    ...
    static readonly u32[] SerialTypeLenTable = new u32[] { 0, 1, 2, 3, 4, 6, 8, 8, 0, 0, 0, 0 };

    static readonly MemRef VdbeRecordCompareScratchMem = new MemRef();
...
if( n>nMaxArgs ) nMaxArgs = n;
    ...
    */
    ...
    //MEM
    static void sqlite3VdbeChangeP4( Vdbe p, int addr, MemRef m, int n )
    {
        ...
        case P4_MEM:
          {
            ...
            {
              sqlite3_snprintf( nTemp, ref zTemp, "%lld", pMem.ui );
            }
...
*/
    static void releaseMemArray( MemRef[] p, int N )
    {
...
*/
    ...
    )
    {
      ...
      int i;
      int rc = SQLITE_OK;
      p.pResultSet = new MemRef[p.nMem];//Mem* pMem = p.pResultSet = p.aMem[1];
      MemRef pMem;
...
////        db.mallocFailed = 1;
      ...
      int i_pMem = 0; if ( p.pResultSet[i_pMem] == null ) p.pResultSet[i_pMem] = new MemRef();
      ...
      else
      {
        ...
        if ( p.explain == 1 )
        {
          ...
          pMem.ui = i;                                /* Program counter */
          if ( p.pResultSet[i_pMem] == null ) p.pResultSet[i_pMem] = new MemRef();
    ...
    )
    {
      ...
      */
      if ( nVar >= 0 /* &&  ALWAYS(db->mallocFailed==0) */ )
      {
        ...
        // C# -- Replace allocation with individual Dims
        p.aMem = new MemRef[nMem + 1];
        for ( n = 0 ; n <= nMem ; n++ )
        { p.aMem[n] = new MemRef(); }//p.aMem--;
        /* aMem[] goes from 1..nMem */
        p.nMem = nMem;      /*       not from 0..nMem-1 */
        //
        p.aVar = new MemRef[nVar == 0 ? 1 : nVar]; //p.aVar = p.aMem[nMem + 1];
        for ( n = 0 ; n < nVar ; n++ )
        { p.aVar[n] = new MemRef(); }
        p.nVar = (u16)nVar;
        p.okVar = 0;
        //
        p.apArg = new MemRef[nArg == 0 ? 1 : nArg];//p.apArg = (Mem**)p.aVar[nVar];
        //
...
p.inVtabMethod = 0;
    ...
    */
    static void sqlite3VdbeSetNumCols( Vdbe p, int nResColumn )
    {
      ...
      p.aColName = new MemRef[n];// (Mem*)sqlite3DbMallocZero(db, Mem.Length * n);
      //if (p.aColName == 0) return;
      while ( n-- > 0 )
      {
        p.aColName[n] = new MemRef();
...
fclose(out);
    ...
    */
    static int sqlite3VdbeCursorMoveto( VdbeCursor p )
    {
      if ( p.deferredMoveto )
      {
        ...
        rc = sqlite3BtreeMovetoUnpacked( p.pCursor, ref UnpackedRecord.Empty, p.movetoTarget, 0, ref res );
        if ( rc != 0 ) return rc;
    ...
    */
    static u32 sqlite3VdbeSerialType( MemRef pMem, int file_format )
    {
    ...
    */
    static u32 sqlite3VdbeSerialTypeLen( u32 serial_type )
    {
      ...
      {
        return SerialTypeLenTable[serial_type];
      }
    }
...
*/
    static u32 sqlite3VdbeSerialPut( byte[] buf, int offset, int nBuf, MemRef pMem, int file_format )
    {
      ...
      if ( serial_type <= 7 && serial_type > 0 )
      {
        ...
        if ( serial_type == 7 )
        {
          ...
          v = (ulong)XBitConverter.DoubleToInt64Bits( pMem.r );// memcpy( &v, pMem.r, v ).Length;
#if  SQLITE_MIXED_ENDIAN_64BIT_FLOAT
swapMixedEndianFloat( v );
      ...
      if ( serial_type >= 12 )
      {
        Debug.Assert( pMem.n + ( ( pMem.flags & MEM_Zero ) != 0 ? pMem.unZero : 0 ) == (int)sqlite3VdbeSerialTypeLen( serial_type ) );
    ...
    MemRef pMem            /* Memory cell to write value into */
    ) {
        return sqlite3VdbeSerialGet(buf, offset, serial_type, ref pMem.inner);
    }
    ...
    */
    ...
    int offset,         /* Offset into Buffer */
    u32 serial_type,    /* Serial type to deserialize */
    ref Mem pMem            /* Memory cell to write value into */
    )
    {
      switch ( serial_type )
      {
        ...
        case 1:
          { /* 1-byte signed integer */
            pMem.ui = (sbyte)buf[offset + 0];
            pMem.flags = MEM_Int;
            return 1;
          }
        case 2:
          { /* 2-byte signed integer */
            pMem.ui = (int)( ( ( (sbyte)buf[offset + 0] ) << 8 ) | buf[offset + 1] );
            pMem.flags = MEM_Int;
            return 2;
          }
        case 3:
          { /* 3-byte signed integer */
            pMem.ui = (int)( ( ( (sbyte)buf[offset + 0] ) << 16 ) | ( buf[offset + 1] << 8 ) | buf[offset + 2] );
            pMem.flags = MEM_Int;
            return 3;
          }
        case 4:
          { /* 4-byte signed integer */
            pMem.ui = (int)( ( (sbyte)buf[offset + 0] << 24 ) | ( buf[offset + 1] << 16 ) | ( buf[offset + 2] << 8 ) | buf[offset + 3] );
            pMem.flags = MEM_Int;
            return 4;
          }
        case 5:
...
swapMixedEndianFloat(x);
#endif
              pMem.r = XBitConverter.Int64BitsToDouble( (long)x );// memcpy(pMem.r, x, sizeof(x))
    ...
    static uint sqlite3VdbeSerialGet(
    byte[] buf,     /* Buffer to deserialize from */
    u32 serial_type,              /* Serial type to deserialize */
    ref Mem pMem                     /* Memory cell to write value into */
    )
    {
      return sqlite3VdbeSerialGet(buf, 0, serial_type, ref pMem);
    }
    ...
    MemRef pMem                     /* Memory cell to write value into */
        ...
        return sqlite3VdbeSerialGet(buf, 0, serial_type, ref pMem.inner);
    }
    ...
    */
    ...
    ref UnpackedRecord pSpace, //  char *pSpace,          /* Unaligned space available to hold the object */
    ...
    )
    {
      ...
      */
      ...
      //}
      //else
      pSpace.flags = UNPACKED_NEED_DESTROY;
      pSpace.pKeyInfo = pKeyInfo;
      pSpace.nField = (u16)(pKeyInfo.nField + 1);
      //p->aMem = pMem = (Mem*)&( (char*)p )[ROUND8( sizeof( UnpackedRecord ) )];
      //assert( EIGHT_BYTE_ALIGNMENT( pMem ) );
      pSpace.aMem = new MemRef[pSpace.nField + 1];
      idx = (u32)getVarint32( aKey, 0, ref szHdr );// GetVarint( aKey, szHdr );
      d = (int)szHdr;
      u = 0;
      while (idx < (int)szHdr && u < pSpace.nField && d <= nKey)
      {
        pSpace.aMem[u] = new MemRef();
        pMem = pSpace.aMem[u];
        u32 serial_type = 0;
    ...
    */
    static void sqlite3VdbeDeleteUnpackedRecord( ref UnpackedRecord p )
    {
    ...
    */
    ...
    {
      return sqlite3VdbeRecordCompare( nKey1, pKey1, 0, ref pPKey2 );
    }
    ...
    */
    static int sqlite3VdbeIdxRowid( sqlite3 db, BtCursor pCur, ref i64 rowid )
    {
      ...
      */
      ...
      m.db = db;
      //m.zMalloc = null;
      rc = sqlite3VdbeMemFromBtree( pCur, 0, (int)nCellKey, true, ref m );
      if ( rc != 0 )
      {
        return rc;
      }
      ...
      ** Verify that the last entry really is an integer. */
      ...
      sqlite3VdbeSerialGet( m.zBLOB, (int)( m.n - lenRowid ), typeRowid, ref v );
      rowid = v.ui;
      sqlite3VdbeMemRelease( ref m );
      return SQLITE_OK;
...
idx_rowid_corruption:
    ...
    */
    ...
    ref UnpackedRecord pUnpacked,   /* Unpacked version of key to compare against */
    ...
    )
    {
      ...
      ** that btreeParseCellPtr() and sqlite3GetVarint32() are implemented */
      ...
      res = sqlite3VdbeRecordCompare( m.n, m.zBLOB, ref pUnpacked );
